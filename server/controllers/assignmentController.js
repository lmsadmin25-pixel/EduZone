...
const createAssignment = async (req, res, next) => {
  try {
    const assignment = await Assignment.create({ ...req.body, educator: req.user._id });
...
const submitAssignment = async (req, res, next) => {
  try {
    ...
    const submission = await Submission.create({ assignment: req.params.id, student: req.user._id, fileUrl });

    await Notification.create({
      user: assignment.educator, userType: 'Educator',
...
const gradeSubmission = async (req, res, next) => {
  try {
    ...
    const submission = await Submission.findByIdAndUpdate(
      req.params.id, { marks, feedback, isGraded: true }, { new: true }
    ).populate('student', 'name email');

    if (!submission) return res.status(404).json({ message: 'Submission not found' });

    await Notification.create({
      user: submission.student._id, userType: 'Student',
...
// @desc    Admin — get all assignments with platform-wide stats
// @route   GET /api/admin/assignments
const adminGetAllAssignments = async (req, res, next) => {
  try {
    ...
    res.json({
      ...
      stats: {
        ...
        pendingSubmissions: allSubmissions.length - gradedSubmissions.length,
        avgScore
      }
    });
  } catch (error) { next(error); }
...
module.exports = {
  ...
  getSubmissions, gradeSubmission, adminGetAllAssignments
};
